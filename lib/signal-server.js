...
class SignalServer extends EventEmitter {
  ...
  onDisconnect(socket) {
    ...
    this.nodeArray = this.nodeArray.filter(e => e !== this.socketToNode[socket.id]);
    delete this.nodeToSocket[this.socketToNode[socket.id]];
    delete this.socketToNode[socket.id];
  ...
  onDiscover(socket, nodeId) {
    ...
    this.socketToNode[socket.id] = nodeId;
  }
