...
class SignalClient extends EventEmitter{
  ...
  constructor(socket, metadata){
    ...
    socket.on('simple-signal[discover]', self.onDiscover.bind(self));
    socket.on('simple-signal[offer]', self.onOffer.bind(self));
    socket.on('simple-signal[answer]', self.onAnswer.bind(self));
    socket.on('simple-signal[dispatch]', self.onDispatch.bind(self));
  ...
  getRandomNodeId (metadata) {
    ...
    self.socket.emit('simple-signal[getRandomNodeId]', metadata);
  }
