...
class SignalClient extends EventEmitter{
  ...
  constructor(socket, nodeID){
    ...
    if (self.socket.connected) {
      self.socket.emit('simple-signal[discover]', self.nodeID);
    }
    ...
    self.socket.on('simple-signal[discover]', self.onDiscover.bind(self));
    self.socket.on('simple-signal[offer]', self.onOffer.bind(self));
    self.socket.on('simple-signal[answer]', self.onAnswer.bind(self));
    self.socket.on('simple-signal[dispatch]', self.onDispatch.bind(self));
  }
  ...
  connect(id, opts, metadata) {
    ...
    peer.on('signal', function (signal) {
      console.log("signal-client: before emit simple-signal[offer]");
  ...
  getRandomNodeId (nodeId) {
    ...
    self.socket.emit('simple-signal[getRandomNodeId]', nodeId);
  }
}
