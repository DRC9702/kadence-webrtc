...
class WebRTCTransport extends DuplexStream {
  ...
  constructor(options) {
    ...
    this.messageClient = new MessageClient(this.socket, this.nodeID.toString('hex'), this, this._options);
  ...
  _write([id, buffer, target], encoding, callback) {
    ...
    this.messageClient.sendMessage(this.nodeID, targetID, buffer, encoding, callback);
  }
